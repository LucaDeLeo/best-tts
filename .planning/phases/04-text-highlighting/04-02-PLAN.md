...
<task type="auto">
  <name>Task 2: Add highlight toggling functions</name>
  <files>src/lib/highlight-manager.ts</files>
  <action>
Add highlight toggling and auto-scroll functions to highlight-manager.ts:

```typescript
// Add to existing highlight-manager.ts
...
These functions will be used by both overlay and selection mode.
  </action>
  <verify>npx tsc --noEmit src/lib/highlight-manager.ts && npm run build</verify>
  <done>Highlight toggling and auto-scroll functions added and compile</done>
</task>
...
2. highlight-manager.ts has highlightSentence, clearHighlight, maybeScrollToSentence
3. npm run build succeeds
4. Overlay container can be created and removed
</verification>
...
- Highlight toggling works via CSS class manipulation
- Auto-scroll respects user scroll override
</success_criteria>
