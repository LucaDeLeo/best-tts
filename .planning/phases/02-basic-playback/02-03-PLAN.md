...
must_haves:
  truths:
    ...
    - "Service worker handles AUDIO_ERROR to reset state and forward error to popup"
  artifacts:
...
case MessageType.AUDIO_ERROR:
  ...
  const errorMsg = message as AudioErrorMessage;
  const errorState = getPlaybackState();
  if (errorMsg.generationToken === errorState.generationToken) {
    console.error('Audio playback error from content script:', errorMsg.error);
